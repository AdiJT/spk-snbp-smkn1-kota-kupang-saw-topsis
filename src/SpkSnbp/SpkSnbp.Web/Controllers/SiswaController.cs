...
public class SiswaController : Controller
{
    ...
    public async Task<IActionResult> PDF(int? tahun, Jurusan? jurusan)
    {
        ...
        var html = await _templateEngine.RenderAsync("Views/Siswa/PDF.cshtml", daftarSiswa);
    ...
    public async Task<IActionResult> Excel(int? tahun, Jurusan? jurusan)
    {
        ...
        headerRow.Append(
            [
                ...
                ..Enumerable.Range(1, daftarKriteria.Count - 1).Select(x => new Cell {
                    CellReference = $"{(char)((int)'E' + x)}{headerRow.RowIndex}",
                    StyleIndex = 2,
                })
            ]
        );
        ...
        headerRow2.Append(
            [
                ...
                ..daftarKriteria.OrderBy(x => x.Id).Select(x => new Cell {
                    CellReference = $"{(char)((int)'D' + x.Id)}{headerRow2.RowIndex}",
            ...
            new MergeCells(
                [
                    ..Enumerable.Range(0, 4)
                        .Select(x => new MergeCell { Reference = $"{(char)((int)'A' + x)}1:{(char)((int)'A' + x)}2"}),
                    new MergeCell { Reference = $"E1:{(char)((int)'E' + daftarKriteria.Count - 1)}1" }
                ]
            ), 
            sheetData
        );
        ...
        for (int i = 0; i < daftarSiswa.Count; i++)
        {
            ...
            [
                ...
                ..daftarKriteria.OrderBy(x => x.Id).Select(x => new Cell {
                    CellReference = $"{(char)((int)'D' + x.Id)}{row.RowIndex}",
