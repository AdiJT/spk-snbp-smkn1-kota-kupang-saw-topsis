// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using SpkSnbp.Infrastructure.Database;

#nullable disable

namespace SpkSnbp.Infrastructure.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20260405102338_TambahActiveKriteria")]
    partial class TambahActiveKriteria
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "9.0.11")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("SpkSnbp.Domain.Auth.User", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Role")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("UserName")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("User");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            PasswordHash = "AQAAAAIAAYagAAAAEKXsR8woVHO5DgmyBgmfe5b4I7jeJZYtk71JFY4HkDSCsimeHtIwzOueTyHo8gBH/A==",
                            Role = "Admin",
                            UserName = "Admin"
                        },
                        new
                        {
                            Id = 2,
                            PasswordHash = "AQAAAAIAAYagAAAAEKXsR8woVHO5DgmyBgmfe5b4I7jeJZYtk71JFY4HkDSCsimeHtIwzOueTyHo8gBH/A==",
                            Role = "Wali Kelas",
                            UserName = "Wali Kelas"
                        },
                        new
                        {
                            Id = 3,
                            PasswordHash = "AQAAAAIAAYagAAAAEKXsR8woVHO5DgmyBgmfe5b4I7jeJZYtk71JFY4HkDSCsimeHtIwzOueTyHo8gBH/A==",
                            Role = "Kepala Sekolah",
                            UserName = "Kepala Sekolah"
                        });
                });

            modelBuilder.Entity("SpkSnbp.Domain.ModulUtama.HasilPerhitungan", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.PrimitiveCollection<double[]>("JarakSolusiIdealNegatif")
                        .IsRequired()
                        .HasColumnType("double precision[]");

                    b.PrimitiveCollection<double[]>("JarakSolusiIdealPositif")
                        .IsRequired()
                        .HasColumnType("double precision[]");

                    b.Property<int>("Jurusan")
                        .HasColumnType("integer");

                    b.Property<double[,]>("MatriksPerhitungan")
                        .IsRequired()
                        .HasColumnType("double precision[]");

                    b.Property<double[,]>("MatriksTernormalisasi")
                        .IsRequired()
                        .HasColumnType("double precision[]");

                    b.Property<double[,]>("MatriksTernormalisasiTerbobot")
                        .IsRequired()
                        .HasColumnType("double precision[]");

                    b.PrimitiveCollection<double[]>("MaxKriteria")
                        .IsRequired()
                        .HasColumnType("double precision[]");

                    b.PrimitiveCollection<double[]>("MinKriteria")
                        .IsRequired()
                        .HasColumnType("double precision[]");

                    b.PrimitiveCollection<double[]>("SolusiIdealNegatif")
                        .IsRequired()
                        .HasColumnType("double precision[]");

                    b.PrimitiveCollection<double[]>("SolusiIdealPositif")
                        .IsRequired()
                        .HasColumnType("double precision[]");

                    b.Property<int>("TahunAjaranId")
                        .HasColumnType("integer");

                    b.Property<DateTime>("TanggalPerhitungan")
                        .HasColumnType("timestamp without time zone");

                    b.HasKey("Id");

                    b.HasIndex("TahunAjaranId");

                    b.ToTable("HasilPerhitungan");
                });

            modelBuilder.Entity("SpkSnbp.Domain.ModulUtama.InformasiSekolah", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Akreditasi")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("BentukPendidikan")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("DesaKelurahan")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Jalan")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("KabupatenKota")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("KecamatanDistrik")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("KepalaSekolah")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("KodePos")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Misi")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("NPSN")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("NamaSekolah")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<double>("Nilai")
                        .HasColumnType("double precision");

                    b.Property<string>("NoHP")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("NoSKAkreditasi")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Provinsi")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateOnly>("TMTMulaiSKAkreditasi")
                        .HasColumnType("date");

                    b.Property<DateOnly>("TMTSelesaiSKAkreditasi")
                        .HasColumnType("date");

                    b.Property<DateOnly>("TanggalSKAkreditasi")
                        .HasColumnType("date");

                    b.Property<string>("Visi")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("InformasiSekolah");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Akreditasi = "A",
                            BentukPendidikan = "SMK",
                            DesaKelurahan = "Oetete",
                            Jalan = "JL. PROF. DR. W. Z. JOHANES",
                            KabupatenKota = "Kota Kupang",
                            KecamatanDistrik = "Kec. Oebobo",
                            KepalaSekolah = "Mixon Rudolf Nicolas Abineno",
                            KodePos = "85112",
                            Misi = "<ol>\r\n                <li>Menumbuhkan semangat keunggulan dan komparatif secara intensif kepada seluruh warga sekolah. </li>\r\n                <li>Melaksanakan kegiatan belajar mengajar secara optimal yang berorientasi kepada pencapaian kompetensi berstandar nasional/internasional dengan tetap mempertimbangkan potensi yang dimiliki peserta didik. </li>\r\n                <li>Mengembangkan dan mengintensifkan hubungan sekolah dengan DUDI dan institusi lain yang telah memiliki reputasi nasional/internasional sebagai perwujudan dari <em>demand driven</em>.</li>\r\n                <li>Menerapkan pengelolaan manajemen SMK mengacu pada standar ISO 9001 2008 dengan melibatkan seluruh warga sekolah dan <em>pelanggan</em>. </li>\r\n            </ol>",
                            NPSN = "50304998",
                            NamaSekolah = "SMKN 1 Kota Kupang",
                            Nilai = 92.0,
                            NoHP = "081313474144",
                            NoSKAkreditasi = "1857/BAN-SM/SK/2022",
                            Provinsi = "Prov. Nusa Tenggara Timur",
                            TMTMulaiSKAkreditasi = new DateOnly(2022, 11, 30),
                            TMTSelesaiSKAkreditasi = new DateOnly(2027, 12, 31),
                            TanggalSKAkreditasi = new DateOnly(2022, 11, 30),
                            Visi = "<p>\r\n                ”Menjadi Lembaga Pendidikan dan Pelatihan Bisnis Manajemen, Pariwisata, dan Teknologi Informatika dan Komunikasi Berstandar Nasional dan Internasional serta Mampu Menghasilkan Tamatan yang Berdaya Saing di 	Era Global.”\r\n            </p>"
                        });
                });

            modelBuilder.Entity("SpkSnbp.Domain.ModulUtama.Kelas", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Nama")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Kelas");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Nama = "1"
                        },
                        new
                        {
                            Id = 2,
                            Nama = "2"
                        },
                        new
                        {
                            Id = 3,
                            Nama = "3"
                        },
                        new
                        {
                            Id = 4,
                            Nama = "4"
                        },
                        new
                        {
                            Id = 5,
                            Nama = "5"
                        });
                });

            modelBuilder.Entity("SpkSnbp.Domain.ModulUtama.Kriteria", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<bool>("Active")
                        .HasColumnType("boolean");

                    b.Property<int>("Bobot")
                        .HasColumnType("integer");

                    b.Property<bool>("IsDefault")
                        .HasColumnType("boolean");

                    b.Property<int>("Jenis")
                        .HasColumnType("integer");

                    b.Property<string>("Nama")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Kriteria");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Active = true,
                            Bobot = 5,
                            IsDefault = true,
                            Jenis = 0,
                            Nama = "Mata Pelajaran Kejuruan"
                        },
                        new
                        {
                            Id = 2,
                            Active = true,
                            Bobot = 4,
                            IsDefault = true,
                            Jenis = 0,
                            Nama = "Mata Pelajaran Umum"
                        },
                        new
                        {
                            Id = 3,
                            Active = true,
                            Bobot = 4,
                            IsDefault = true,
                            Jenis = 0,
                            Nama = "Sertifikat LSP"
                        },
                        new
                        {
                            Id = 4,
                            Active = true,
                            Bobot = 3,
                            IsDefault = true,
                            Jenis = 0,
                            Nama = "Sertifikat TKA"
                        },
                        new
                        {
                            Id = 5,
                            Active = true,
                            Bobot = 2,
                            IsDefault = true,
                            Jenis = 0,
                            Nama = "Ekstrakurikuler"
                        },
                        new
                        {
                            Id = 6,
                            Active = true,
                            Bobot = 1,
                            IsDefault = true,
                            Jenis = 1,
                            Nama = "Absensi"
                        });
                });

            modelBuilder.Entity("SpkSnbp.Domain.ModulUtama.Siswa", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int?>("Ekstrakulikuler1")
                        .HasColumnType("integer");

                    b.Property<int?>("Ekstrakulikuler2")
                        .HasColumnType("integer");

                    b.Property<int?>("Ekstrakulikuler3")
                        .HasColumnType("integer");

                    b.Property<int?>("Eligible")
                        .HasColumnType("integer");

                    b.Property<int?>("HasilPerhitunganId")
                        .HasColumnType("integer");

                    b.Property<int?>("JumlahAbsen")
                        .HasColumnType("integer");

                    b.Property<int>("Jurusan")
                        .HasColumnType("integer");

                    b.Property<int>("KelasId")
                        .HasColumnType("integer");

                    b.Property<string>("NISN")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Nama")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<double?>("NilaiTopsis")
                        .HasColumnType("double precision");

                    b.Property<double?>("SkorTKA")
                        .HasColumnType("double precision");

                    b.Property<int>("TahunAjaranId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("HasilPerhitunganId");

                    b.HasIndex("KelasId");

                    b.HasIndex("TahunAjaranId");

                    b.ToTable("Siswa");
                });

            modelBuilder.Entity("SpkSnbp.Domain.ModulUtama.SiswaKriteria", b =>
                {
                    b.Property<int>("IdSiswa")
                        .HasColumnType("integer");

                    b.Property<int>("IdKriteria")
                        .HasColumnType("integer");

                    b.Property<double>("Nilai")
                        .HasColumnType("double precision");

                    b.HasKey("IdSiswa", "IdKriteria");

                    b.HasIndex("IdKriteria");

                    b.ToTable("SiswaKriteria");
                });

            modelBuilder.Entity("SpkSnbp.Domain.ModulUtama.TahunAjaran", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.HasKey("Id");

                    b.ToTable("TahunAjaran");

                    b.HasData(
                        new
                        {
                            Id = 2025
                        });
                });

            modelBuilder.Entity("SpkSnbp.Domain.ModulUtama.HasilPerhitungan", b =>
                {
                    b.HasOne("SpkSnbp.Domain.ModulUtama.TahunAjaran", "TahunAjaran")
                        .WithMany("DaftarHasil")
                        .HasForeignKey("TahunAjaranId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("TahunAjaran");
                });

            modelBuilder.Entity("SpkSnbp.Domain.ModulUtama.Siswa", b =>
                {
                    b.HasOne("SpkSnbp.Domain.ModulUtama.HasilPerhitungan", "HasilPerhitungan")
                        .WithMany("DaftarSiswa")
                        .HasForeignKey("HasilPerhitunganId");

                    b.HasOne("SpkSnbp.Domain.ModulUtama.Kelas", "Kelas")
                        .WithMany("DaftarSiswa")
                        .HasForeignKey("KelasId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SpkSnbp.Domain.ModulUtama.TahunAjaran", "TahunAjaran")
                        .WithMany("DaftarSiswa")
                        .HasForeignKey("TahunAjaranId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("HasilPerhitungan");

                    b.Navigation("Kelas");

                    b.Navigation("TahunAjaran");
                });

            modelBuilder.Entity("SpkSnbp.Domain.ModulUtama.SiswaKriteria", b =>
                {
                    b.HasOne("SpkSnbp.Domain.ModulUtama.Kriteria", "Kriteria")
                        .WithMany("DaftarSiswaKriteria")
                        .HasForeignKey("IdKriteria")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("SpkSnbp.Domain.ModulUtama.Siswa", "Siswa")
                        .WithMany("DaftarSiswaKriteria")
                        .HasForeignKey("IdSiswa")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Kriteria");

                    b.Navigation("Siswa");
                });

            modelBuilder.Entity("SpkSnbp.Domain.ModulUtama.HasilPerhitungan", b =>
                {
                    b.Navigation("DaftarSiswa");
                });

            modelBuilder.Entity("SpkSnbp.Domain.ModulUtama.Kelas", b =>
                {
                    b.Navigation("DaftarSiswa");
                });

            modelBuilder.Entity("SpkSnbp.Domain.ModulUtama.Kriteria", b =>
                {
                    b.Navigation("DaftarSiswaKriteria");
                });

            modelBuilder.Entity("SpkSnbp.Domain.ModulUtama.Siswa", b =>
                {
                    b.Navigation("DaftarSiswaKriteria");
                });

            modelBuilder.Entity("SpkSnbp.Domain.ModulUtama.TahunAjaran", b =>
                {
                    b.Navigation("DaftarHasil");

                    b.Navigation("DaftarSiswa");
                });
#pragma warning restore 612, 618
        }
    }
}
